...
with st.sidebar.expander("🧚‍♀️ ถาม-ตอบ กับไมล่า (Q&A)", expanded=True):
    ...
    # [UPDATED] ข้อมูลใหม่เกี่ยวกับไมล่า
        ...
        if st.button("ไขกุญแจ"):
            try:
                ...
                if username.strip() == real_user and password.strip() == real_pass:
                    ...
                    st.sidebar.error("ผิดครับ! ลองเช็คตัวเล็กตัวใหญ่ดูนะ 😅")
            except:
                st.sidebar.error("ระบบตรวจสอบผิดพลาด ลองใหม่ครับ")
else:
